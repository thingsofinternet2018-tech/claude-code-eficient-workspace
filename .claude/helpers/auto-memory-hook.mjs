...
async function loadMemoryPackage() {
  // Strategy 1: Local dev (built dist) — fast check, no fallback walk
  ...
  // Strategy 2: Direct node_modules (most common install path)
  const directNm = join(PROJECT_ROOT, 'node_modules', '@claude-flow', 'memory', 'dist', 'index.js');
  if (existsSync(directNm)) {
    try { return await import(`file://${directNm}`); } catch { /* fall through */ }
  }

  // Strategy 3: ESM import (package.json dep)
  try { return await import('@claude-flow/memory'); } catch { /* fall through */ }

  // Fast-fail: nu mai căuta în toată ierarhia — dacă nu e în locurile evidente, nu e instalat
  return null;
}
