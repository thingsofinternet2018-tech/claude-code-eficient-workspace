...
## [3.10.0] — 2026-06-10 — Upstream of 4 weeks of empirically validated lessons from the live workspace

### New rules (_SETTINGS/RULES/)
- **`protocol_paired_mesh_agents.md`** — N divergent + N convergent agents in ONE message before any complex investigation/decision; bans serial fix-build-test loops. Validated on a production Android hang (1 round vs 4).
- **`cold_audit_post_pipeline.md`** — mandatory cold audit (3 context-free agents: reader / regression-vs-backup / strict-rules) before declaring any ≥3-batch pipeline "done". Lesson: regex-clean ≠ semantically clean.
- **`no_auto_delete_external_storage.md`** — never auto-delete on NAS/SMB/SSHFS/FTP paths or drive mappings; "cleanup" is not approval.
- **`llm_words_eliminate.md`** — 8 decorative LLM word families (pure/absolute/essential/real/profound/ultimate/genuine/vast) replaced with concrete directional phrases; bulk changes require divergent-convergent validation.
- **`rag_auto_query_on_uncertainty.md`** — RAG corpus as automatic tie-breaker (4th wing) on agent non-consensus; full-fragment queries; multi-zoom iterative consultation; placeholder IDs only (never commit real notebook IDs).

### BEST_PRACTICES.md
- **PATTERN-010** Cold audit before "done" · **PATTERN-011** All independent agents in ONE message, distinct wings, dedicated convergent at ≥3.
- **ANTI-007** Windows hooks: no `cmd /c` double-wrap, absolute paths only, `MSYS_NO_PATHCONV=1` for adb push · **ANTI-008** RAG verdict must pass the project style filter before application.
- **NotebookLM section rewritten:** CLI channel is canonical (`nlm notebook query`, `--timeout 150-180`, `--json`, throttle ≥3s); MCP server dropped after connect/disconnect churn crashes.

### RULES/INDEX.md
- 6 new router rows (paired mesh, cold audit, external-storage delete guard, LLM words, RAG tie-breaker).

### Privacy
- All content sanitized: no notebook IDs, no IPs/hostnames, no personal paths or project names from the source workspace.
...
## [3.8.1] — 2026-05-10 — System audit + AgentDB sync + bridge agent patch

### Highlights
- **WASM fix** — `sql-wasm.wasm` lipsea din cache npx → copiat; AgentDB operațional
...
- **Status audit** — raport complet toate prioritățile → `_MEMORY/sessions/`
