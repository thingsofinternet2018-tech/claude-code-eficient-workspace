...
**De ce:** Ai trimis comenzi la un serviciu fara sa verifici mai intai daca e accesibil. Agentii paraleli elimina asumptiile gresite inainte de actiune.
...
**Refs:** `memory/feedback_investigate_before_act.md`.
...
### PATTERN-010 — Cold audit before declaring "done" (anti-cosmetic)
**Cand:** Any pipeline with ≥3 batches or ≥20 applied changes, before declaring COMPLETE / before a major rebuild or release.
**Ce fac:** Spawn 3 INDEPENDENT agents with zero session context: naive reader/consumer + regression-vs-backup + strict-rules grep. Honest convergent report splits claims into confirmed-real vs partial/false.
**De ce:** Empirically, a pipeline declared "canonical after 3 validation rounds" still contained 5 real semantic problems (2 introduced by the pipeline itself). Agents with session context are biased toward KEEP. "Regex-clean ≠ semantically clean."
**Refs:** `_SETTINGS/RULES/cold_audit_post_pipeline.md`.

### PATTERN-011 — All independent agents in ONE message (no serial waves)
**Cand:** Any time ≥2 independent sub-tasks can run in parallel.
**Ce fac:** Spawn ALL independent agents in a single message (multiple Agent invokes), each with EXCLUSIVE scope and a DISTINCT wing (zoom/lens/perspective). Never the same prompt N times (homogeneous outputs = N redundant agents). At ≥3 agents add a dedicated convergent that synthesizes by 2/3 majority.
**De ce:** Serial waves waste wall-clock and cache; identical prompts waste agents. Validated repeatedly: distinct wings produce cross-confirmed findings a single angle misses.
**Refs:** `_SETTINGS/RULES/multi_agent_divergent_convergent.md`, `_SETTINGS/RULES/protocol_paired_mesh_agents.md`.

### ANTI-007 — Windows hooks: `cmd /c` double-wrap si path-uri relative
**Gresit:** Hook command `cmd /c node script.js` (harness already wraps → quoting breaks, spam errors) sau hook Python cu path relativ / `%CLAUDE_PROJECT_DIR%` (nu se expandeaza).
**Corect:** Invoke direct (`node <absolute path>` / `"<absolute path>.cmd"`); Python hooks cu path ABSOLUT. Pentru adb push din Git-Bash: `MSYS_NO_PATHCONV=1` obligatoriu.
**Refs:** validated fixes — hook handler normalization + settings.json cleanup.

### ANTI-008 — RAG verdict aplicat fara filtrul propriu de stil
**Gresit:** Corpusul RAG (NotebookLM etc.) raspunde, iar raspunsul e aplicat direct in text/cod.
**Corect:** Verdictul RAG trece INTAI prin filtrul de reguli al proiectului (punctuatie interzisa, constructii interzise, format canonic). RAG e tie-breaker pe FOND, nu autoritate pe FORMA.
**Refs:** `_SETTINGS/RULES/rag_auto_query_on_uncertainty.md`.
...
### NotebookLM (RAG pe corpus mare) — canal CLI, NU MCP
- **Unde:** Orice proiect cu >50 documente (carte, glosar, research, legal).
- **Cum (canonic, validat):** CLI cu proces curat per apel:
  `PYTHONIOENCODING=utf-8 nlm notebook query "<id>" "<full source fragment + question>" --timeout 180`
  - Sintaxa corecta e `nlm notebook query` (NU `nlm query notebook` — comanda inexistenta, cauza istorica a "NLM nu raspunde").
  - Query-urile iau 60-120s → `--timeout 150-180`; sub 90s pare fals "mort". `--json` = output structurat (answer + citations).
  - `nlm notebook list` = verificare rapida auth. Throttle ≥3s intre queries, max ~10/sesiune.
  - Trimite MEREU fragmentul-sursa integral, nu doar intrebarea (altfel raspuns generic, citatii lipsa).
- **De ce CLI si nu MCP:** serverul MCP notebooklm face connect/disconnect churn → crash-uri. Validat: scos din config, canal oficial = CLI.
